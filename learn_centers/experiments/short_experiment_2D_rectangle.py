...
def launch_experiment(train_size: int, centers_size: int, fn_name: str,
                      ...
                      shape: float, xmin: float, xmax: float, lr: float,
                      use_scheduler: bool):
    # Load function
    ...
    folder = 'dim_2/numerics_rectangle'
    ...
    n_boundary_points = 4 * (train_size - 2)
    x_train = get_rectangle(bottom_left=(xmin, xmin), width=xmax, height=xmax,
                            n_points_per_boundary=n_boundary_points // 4,
                            n_interior_points=train_size**2-n_boundary_points,
                            separate_into_sets=False, requires_grad=True).to('cuda')
    y_train = fn(x_train).reshape(-1, 1)
    x_validate = torch.cartesian_prod(
        *[torch.linspace(xmin, xmax, n_verification) for _ in range(dim)]).reshape(-1, dim).to('cuda')
    ...
    if poly_degree > -1:
        nn = RBF_Poly(input_dim=dim, num_centers=centers_size, output_dim=1, kernel=kernel_fn,
                      left_lim=xmin, right_lim=xmax, starting_shape=shape, degree=poly_degree,
                      dev='cuda').to('cuda')
        nn.rbf.set_centers(torch.cartesian_prod(
            *[torch.linspace(xmin, xmax, floor(sqrt(centers_size)), device='cuda'),
              torch.linspace(xmin, xmax, floor(sqrt(centers_size)), device='cuda')]).reshape(-1, 2))

        def get_shape(): return nn.rbf.shape
    else:
        nn = RBF(input_dim=dim, num_centers=centers_size,
                 output_dim=1, kernel=kernel_fn, left_lim=xmin, right_lim=xmax, starting_shape=shape).to('cuda')
    ...
    while True:
        ...
        if (epoch > 0 and epoch % save_every == 0 and save_every > 0) or (math.isclose(lr_list[-1], min_lr, abs_tol=min_lr/10)):
            plot_data_3d(centers=nn.get_centers().cpu(), xmin=xmin, xmax=xmax,
                         approx=nn_validation_output,
                         target=y_validate, res=n_verification,
                         filename=f'{experiment_file}-E{epoch}.pdf',
    ...
    save_curves(f'{folder}/curves-{fn_name}-TR{train_size}-C{centers_size}{poly_suffix}-tag{tag}-E{epoch}-curves.csv',
                loss_list, l2_list, linf_list, lr_list, x_train.cpu().detach().numpy().tolist())
...
parser.add_argument("--fn_name", type=str,
                    ...
                    required=is_required, default='franke_function')
...
launch_experiment(train_size=args.train_size, centers_size=args.center_size,
                  ...
                  xmin=args.xmin, xmax=args.xmax, tag=args.tag, lr=args.lr)
