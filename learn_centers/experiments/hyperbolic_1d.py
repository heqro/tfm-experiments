...
def launch_experiment(n_training_points_1d: int, n_total_centers: int, tag: str, save_every: int,
                      ...
                      sol: str = 'hyperbolic_1d', xmax: float = 1., use_scheduler: bool = True):

    def interior_expr(model: RBF,
                      x_pts: torch.Tensor, t_pts: torch.Tensor) -> torch.Tensor:
    ...
    def total_loss(model: RBF, x_int: torch.Tensor, y_int: torch.Tensor,
        ...
        loss_interior = weight_interior * \
            interior_expr(model, x_int, y_int)
    ...
    int_train = torch.cat((rectangle_dict['interior'], rectangle_dict['boundary']['top'],  # type: ignore
                           rectangle_dict['boundary']['right'], rectangle_dict['corners']['top_right']),  # type: ignore
                          dim=0).to('cuda')

    # x, y = int_train[:, 0], int_train[:, 1]
    # cond1 = y >= (x-.2)
    # cond2 = y <= (x+.2)
    # cond = cond1 & cond2
    # int_train = int_train[cond]

    if not isinstance(int_train, torch.Tensor):
        raise Exception('The interior points should be torch.Tensor')

    bd_train = torch.cat((rectangle_dict['boundary']['left'],  # type: ignore
                          rectangle_dict['boundary']['bottom'],  # type: ignore
                          rectangle_dict['corners']['bottom_left'], rectangle_dict['corners']['top_left'], rectangle_dict['corners']['bottom_right']),  # type: ignore
                         dim=0).to('cuda')
    ...
    while True:
        ...
        loss_int, loss_bd = total_loss(model=nn, x_int=int_train[:, 0], y_int=int_train[:, 1],
                                       xy_boundary=bd_train, f_boundary=bd_target)
        ...
        if (epoch > 0 and epoch % save_every == 0 and save_every > 0) or (math.isclose(lr_list[-1], min_lr, abs_tol=min_lr/10)):
            ...
            save_to_csv(experiment_file=f'{experiment_file}-E{epoch}.csv', kernel_name=kernel_name, nn=nn,
                        shape=shape, L_inf_norm_epoch=L_inf_norm_epoch, get_shape=get_shape, elapsed_time=-1,
...
parser.add_argument("--train_size", type=int,
                    help="Number of 1d training points", required=is_required, default=100)
...
parser.add_argument("--fn_name", type=str,
                    required=is_required, default='hyperbolic_1d')
