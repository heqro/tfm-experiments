...
def launch_experiment(train_size: int, centers_size: int, save_every: int,
    ...
    def train_model(linf_list, loss_int_list, loss_bd_list, l2_list, lr_list, epoch, L_inf_norm_epoch):
        while True:
    ...
    with torch.no_grad():  # preconditioning phase: fixed centers, common shape parameter
        nn.get_centers().requires_grad = False
        nn.set_shapes(torch.nn.Parameter(torch.Tensor([shape])).to('cuda'))
    ...
    if use_scheduler:
        scheduler = ReduceLROnPlateau(
            optimizer, mode='min', factor=0.1, patience=20, threshold=1e-4, verbose=True, min_lr=min_lr)
    ...
    epoch, L_inf_norm_epoch = train_model(linf_list, loss_int_list,
                                          loss_bd_list, l2_list, lr_list, epoch, L_inf_norm_epoch)
    with torch.no_grad():  # post-conditioning phase: movable centers, separate shape parameter
        print(
            f'Commencing well-conditioned phase with eps={nn.get_shapes().item()}')
        nn = RBF(input_dim=dim, num_centers=centers_size,
                 output_dim=1, kernel=kernel_fn, starting_shape=nn.get_shapes().item()).to('cuda')
        nn.set_centers(torch.linspace(xmin, xmax, centers_size,
                       device='cuda').reshape(-1, 1))
...
parser.add_argument("--train_size", type=int,
                    help="Size of training data", required=is_required, default=240)
parser.add_argument("--center_size", type=int,
                    help="Number of equispaced centers", required=is_required, default=60)
...
parser.add_argument('--polynomial_degree', type=int,
                    default=-1, required=False)
parser.add_argument('--shape', type=float, default=10.74)
