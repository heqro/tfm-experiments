...
\begin{definition}[Multilayer perceptron]
  ...
  %      (\tmmathbf{x}) +\tmmathbf{b}^L
  %    \end{array}, \]
  where $\mathcal{N}^{L - 1} : \mathbb{R}^{N_0} \rightarrow \mathbb{R}^{N_{L
  - 1}}$ is defined according to the rule
  \[ \mathcal{N}^{L - 1} (\tmmathbf{x}) = \left\{\begin{array}{ll}
     ...
     \end{array}\right. . \]
  We shall refer to some $\tmmathbf{b}^i \in \mathbb{R}^{\mathbb{N}_i}, 1 \leq
  i \leq L$ as the {\textbf{bias}} of the $i -$th layer, and each
...
\begin{proof}
  {\todo[inline]{evaluar importancia}}
  \begin{enumerate}
    \item
...
{\todo[inline]{premisa: c{\'o}mo construimos strictly positive definite functions?}}

% \subsection*{Characterizations in terms of Fourier transform}
...
\todo[inline]{this is a cumbersome way of working, which motivates...}
...
% \todo[inline]{comentar el caso de la phs function}
