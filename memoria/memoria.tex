...
Alternatively to this methodology, we propose to let the centers as well as the shape parameters\footnote{We remark that we are now speaking in plural.} be found following an \textbf{unsupervised learning} strategy, whereby the training points will not only dictate the values of $\hat{\lambda}$ as in \eqref{eqn-linear-system-equations-rbf}, but also the position of the RBF centers as well as the value of the shape parameters. \todo[inline]{Explicitly locate the files in the repository} 
...
One reasonable concern for solving any experiment several simultaneous times is that of exhausting the computing resources of the server. The multiple processors the server features is enough to quell any such concern, as long as we launch less problems than processors we have. On the other hand, model memory allocation problem is often an issue when dealing with PINNs, which we address in the following by turning to the \href{https://pytorch.org/docs/stable/generated/torch.cuda.max_memory_allocated.html}{\texttt{max\_memory\_allocated()}}
...
\subsubsection*{About the addition of polynomial terms}

Prior to the experimentation, the incorporation of polynomial addends has previously been seen to have a positive impact regarding the precision of the interpolation methods. However, the preliminary experimentation failed to decisively show the advantages of incorporating them. Therefore, the motivation of this section is to consider polynomial functions and apply our ``statistical'' methodology to try and see if the addition of polynomial terms to the Gaussian kernel does improve the interpolation results on the verification dataset.

To this end, we consider $x \mapsto u_2(x)=(x-1/2)^2, x\in[-1,1]$. We interpolate this function for the same configuration of centers and training points as in the last set of experiments. Furthermore, we consider the Gaussian kernel alongside no polynomial terms, a bias term, a linear polynomial, a quadratic polynomial, and a cubic polynomial. 

We will also consider two different starting shape parameters: the usual $\varepsilon=1$ and $\varepsilon=10$. Recall (see e.g. Figure \ref{fig:negative-exp-rbf}) that the shape parameter plays an important role on the resulting interpolator, with larger values resulting in starker cusps. Not only that, larger shape parameters result in better-posed interpolation problems, which may ultimately improve the results of the interpolation experiments.

\textbf{Starting shape parameter $\varepsilon=1$}

Figure \ref{fig:medians-epochs-statistic-u2-poly-shape-1} shows the medians of the best epochs' ratios as further polynomial terms are considered, whereas Figure \ref{fig:medians-linf-statistic-u2-poly-shape-1} shows the corresponding medians in the $L^\infty-$norm on the verification dataset. On the basis of the $L^\infty-$norm, some conclusions can already be drawn:

\begin{itemize}
  \item When no polynomial term is considered, larger values of $k$ improve the results on the verification dataset, especially with $7$ centers, achieving an improvement of a unit. For larger numbers of centers, the addition of more training points does not have a harmful effect on the interpolation, but produces slightly worse results. The results on the epochs' ratios are as expected, quickly approaching $1$ as $k$ is increased.
  \item With the addition of a bias term, the results with $13$ and $15$ centers see their largest improvement. However, the epochs' ratios do not approach $1$ anymore. The same tendency is observed for higher degree polynomials.
  \item The consideration of a degree $2$ polynomial introduces the most notable improvement, whereby (except for the case involving $7$ centers and $k=1$) almost four digits of accuracy are provided.
\end{itemize}
...
\begin{figure}[H]
  ...
  {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_epochs_u2_gaussian_kernel_shape_1_Poly-1.pdf}}
  {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_epochs_u2_gaussian_kernel_shape_1_Poly0.pdf}}
    {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_epochs_u2_gaussian_kernel_shape_1_Poly1.pdf}}
    {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_epochs_u2_gaussian_kernel_shape_1_Poly2.pdf}}
    {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_epochs_u2_gaussian_kernel_shape_1_Poly3.pdf}}
  \caption{Medians of best epochs' ratios for $\varepsilon=1$.}
  \label{fig:medians-epochs-statistic-u2-poly-shape-1}
\end{figure}

\begin{figure}[H]
  
  {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_linf_u2_gaussian_kernel_shape_1_Poly-1.pdf}}
  {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_linf_u2_gaussian_kernel_shape_1_Poly0.pdf}}
    {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_linf_u2_gaussian_kernel_shape_1_Poly1.pdf}}
    {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_linf_u2_gaussian_kernel_shape_1_Poly2.pdf}}
    {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_linf_u2_gaussian_kernel_shape_1_Poly3.pdf}}
  \caption{Medians of the $\log_{10}(L^\infty-\text{norm})$ for $\varepsilon=1$.}
  \label{fig:medians-linf-statistic-u2-poly-shape-1}
\end{figure}
...
\begin{figure}[H]
  \begin{tabular}{cc}
    \multicolumn{2}{c}{{\includegraphics[width=.7\textwidth, trim={0.5cm 4cm .5cm 3.8cm},clip=true]
          {imagenes/experiments/1d/statistical_1d_full/runge_function/runge_function_legend.pdf}}} \\ 
        With bias term  & With quadratic polynomial \\
    {\includegraphics[width=.5\textwidth, trim={0 1.25cm 0 0},clip=true]
    {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C7_gaussian_kernel_shape_1_Poly0.pdf}}  & 
    {\includegraphics[width=.44\textwidth, trim={.65cm 1.15cm 0 0},clip=true]
    {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C7_gaussian_kernel_shape_1_Poly2.pdf}}                                                                                      \\
    {\includegraphics[width=.5\textwidth, trim={0 1.25cm 0 0},clip=true]
    {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C9_gaussian_kernel_shape_1_Poly0.pdf}}  & 
    {\includegraphics[width=.44\textwidth, trim={.65cm 1.15cm 0 0},clip=true]
        {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C9_gaussian_kernel_shape_1_Poly2.pdf}}
    \\
    {\includegraphics[width=.5\textwidth, trim={0 1.25cm 0 0},clip=true]
    {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C11_gaussian_kernel_shape_1_Poly0.pdf}} & 
    {\includegraphics[width=.44\textwidth, trim={.65cm 1.15cm 0 0},clip=true]
    {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C11_gaussian_kernel_shape_1_Poly2.pdf}}                                                                                     \\
    {\includegraphics[width=.5\textwidth, trim={0 1.25cm 0 0},clip=true]
    {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C13_gaussian_kernel_shape_1_Poly0.pdf}} & 
    {\includegraphics[width=.44\textwidth, trim={.65cm 1.15cm 0 0},clip=true]
        {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C13_gaussian_kernel_shape_1_Poly2.pdf}}
    \\
    {\includegraphics[width=.5\textwidth, trim={0 0.2cm 0 0},clip=true]
    {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C15_gaussian_kernel_shape_1_Poly0.pdf}}
                                                                                                              & 
  {\includegraphics[width=.44\textwidth, trim={.65cm 0.2cm 0 0},clip=true]
        {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C15_gaussian_kernel_shape_1_Poly2.pdf}}
  \end{tabular}
  \caption{$\log_{10}(L^\infty-\text{norm})$ results for the interpolation of \texttt{u2} alongside a Gaussian kernel, when adding a bias term (left) and when adding a quadratic polynomial (right).}
  \label{fig:linf-shape1-poly0-poly2}
\end{figure}
\clearpage
\textbf{Starting shape parameter $\varepsilon=10$}

Figure \ref{fig:medians-epochs-statistic-u2-poly-shape-10} shows the medians of the best epochs' ratios as further polynomial terms are considered, whereas Figure \ref{fig:medians-linf-statistic-u2-poly-shape-10} shows the corresponding medians in the $L^\infty-$norm on the verification dataset. On the basis of the $L^\infty-$norm, some conclusions can already be drawn:

\begin{itemize}
  \item The situation is known already when no polynomial term is considered: the medians produce better results as more centers are considered, also finding a  plateau for larger values of $k$.
  \item The explicit incorporation of a bias term has a positive effect on the interpolation, reducing the previously-observed plateau while providing another digit of accuracy in the best-case scenario. This plateau is further reduced when considering a linear polynomial.
  \item The consideration of a quadratic or cubic polynomial, which have the capabilities of faithfully capturing the function we interpolate, yields about the same medians for $k=4$. This indicates that the incorporation of RBF interpolators featuring a large number of centers is not necessary when we have enough information.
  \item There is a visible change in behavior on the epochs' ratios as well when a quadratic/cubic polynomial is added in.
  \item Figure \ref{fig:linf-shape10-poly0-poly2} shows the violin plots with an explicit bias term alongside a quadratic polynomial. Even though the medians are close, the actual distribution of all the experiments show that those involving a quadratic polynomial (and, to our observations, with $7$ centers) can produce results to machine precision, indeed copying the actual function we sample. As the interpolator gets more complex, featuring more RBF addends, we observe that finding these solutions are less likely, being the worst case in this respect that of $15$ centers.
\end{itemize}
...
\begin{figure}[H]
  ...
  {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_epochs_u2_gaussian_kernel_shape_10_Poly-1.pdf}}
  {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_epochs_u2_gaussian_kernel_shape_10_Poly0.pdf}}
    {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_epochs_u2_gaussian_kernel_shape_10_Poly1.pdf}}
    {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_epochs_u2_gaussian_kernel_shape_10_Poly2.pdf}}
    {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_epochs_u2_gaussian_kernel_shape_10_Poly3.pdf}}
  \caption{Medians of best epochs' ratios for $\varepsilon=10$.}
  \label{fig:medians-epochs-statistic-u2-poly-shape-10}
\end{figure}

\begin{figure}[H]
  
  {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_linf_u2_gaussian_kernel_shape_10_Poly-1.pdf}}
  {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_linf_u2_gaussian_kernel_shape_10_Poly0.pdf}}
    {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_linf_u2_gaussian_kernel_shape_10_Poly1.pdf}}
    {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_linf_u2_gaussian_kernel_shape_10_Poly2.pdf}}
    {\includegraphics[height=.45\textwidth]
    {imagenes/experiments/1d/statistical_1d_full/u2/medians_linf_u2_gaussian_kernel_shape_10_Poly3.pdf}}
  \caption{Medians of the $\log_{10}(L^\infty-\text{norm})$ for $\varepsilon=10$.}
  \label{fig:medians-linf-statistic-u2-poly-shape-10}
\end{figure}
...
\begin{figure}[h]
  \begin{tabular}{cc}
    \multicolumn{2}{c}{{\includegraphics[width=.7\textwidth, trim={0.5cm 4cm .5cm 3.8cm},clip=true]
        ...
        With bias term  & With quadratic polynomial \\
    {\includegraphics[width=.5\textwidth, trim={0 1.25cm 0 0},clip=true]
    {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C7_gaussian_kernel_shape_10_Poly0.pdf}}  & 
    {\includegraphics[width=.44\textwidth, trim={.65cm 1.15cm 0 0},clip=true]
    {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C7_gaussian_kernel_shape_10_Poly2.pdf}}                                                                                      \\
    {\includegraphics[width=.5\textwidth, trim={0 1.25cm 0 0},clip=true]
    {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C9_gaussian_kernel_shape_10_Poly0.pdf}}  & 
    {\includegraphics[width=.44\textwidth, trim={.65cm 1.15cm 0 0},clip=true]
        {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C9_gaussian_kernel_shape_10_Poly2.pdf}}
    \\
    {\includegraphics[width=.5\textwidth, trim={0 1.25cm 0 0},clip=true]
    {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C11_gaussian_kernel_shape_10_Poly0.pdf}} & 
    {\includegraphics[width=.44\textwidth, trim={.65cm 1.15cm 0 0},clip=true]
    {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C11_gaussian_kernel_shape_10_Poly2.pdf}}                                                                                     \\
    {\includegraphics[width=.5\textwidth, trim={0 1.25cm 0 0},clip=true]
    {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C13_gaussian_kernel_shape_10_Poly0.pdf}} & 
    {\includegraphics[width=.44\textwidth, trim={.65cm 1.15cm 0 0},clip=true]
        {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C13_gaussian_kernel_shape_10_Poly2.pdf}}
    \\
    {\includegraphics[width=.5\textwidth, trim={0 0.2cm 0 0},clip=true]
    {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C15_gaussian_kernel_shape_10_Poly0.pdf}}
                                                                                                              & 
  {\includegraphics[width=.44\textwidth, trim={.65cm 0.2cm 0 0},clip=true]
        {imagenes/experiments/1d/statistical_1d_full/u2/linf_u2_C15_gaussian_kernel_shape_10_Poly2.pdf}}
  \end{tabular}
  \caption{$\log_{10}(L^\infty-\text{norm})$ results for the interpolation of \texttt{u2} alongside a Gaussian kernel, when adding a bias term (left) and when adding a quadratic polynomial (right).}
  \label{fig:linf-shape10-poly0-poly2}
\end{figure}

\clearpage

\textbf{Common observations and conclusions}

\begin{itemize}
  \item Figure \ref{fig:distribution-alphas-u2} shows the distribution of our model's shape parameters at the end of the interpolation processes. It shows that when training with a starting shape $\varepsilon=1$, we expect most of the trained shape parameters around the interval $[0,3]$. When starting out with $\varepsilon=10$, the largest incidence of these values is found for $\varepsilon < 10$.
  \begin{figure}[h]
    \includegraphics[width=.5\textwidth]{imagenes/experiments/1d/statistical_1d_full/u2/overall_histogram_of_shapes.pdf}
    \caption{Distribution of shape parameters at the end of the training process, for starting shape parameters $\varepsilon=1$ and $\varepsilon=10$.}
    \label{fig:distribution-alphas-u2}
  \end{figure}
  \item Figures \ref{fig:u2-TR7-C7-Kgaussian_kernel-Poly0-Equi-Sh1-E50000-4} and \ref{fig:u2-TR7-C7-Kgaussian_kernel-Poly0-Equi-Sh10-E50000-34} depict some descriptive realizations of the experiments for the case $C=7,k=1$ with a bias term. They are realizations for shape parameters $\varepsilon=1$ and $\varepsilon=10$ respectively. When there is not enough information, $\varepsilon=10$ exhibits high oscillations between any two interpolation nodes, which produces bad results in the $L^\infty-$norm. However, because we know $\varepsilon=10$ forms a better-posed problem that $\varepsilon=1$, this also allows the loss term to be of the order of $10^{-14}$, whereas the case $\varepsilon=1$ is not able to reduce the loss term as visibly.
  \item This is not just a particular incidence: as $C$ and $k$ increase, Figures \ref{fig:medians-linf-statistic-u2-poly-shape-1} and \ref{fig:medians-linf-statistic-u2-poly-shape-10} show that the case $\varepsilon=1$ produces better results in the $L^\infty-$norm as well, most notably for $k$ close to $1$.
  \item The epochs' ratios (Figures \ref{fig:medians-linf-statistic-u2-poly-shape-1} and \ref{fig:medians-linf-statistic-u2-poly-shape-10}) indicate that, when considering a polynomial term of degree $2$ or $3$, the best epochs in the $L^\infty-$norm tend to happen at about the same iterations, irrespectively of the training points. This implies that, even though the polynomial term may not always guarantee better results in the $L^\infty-$norm, it provides some ``predictability'' beyond the parameters $C$ and $k$ we choose.
  \item The action of the quadratic polynomial term, by which it may produce results to machine precision, appears most notably for $C=7$, both for $\varepsilon=1$ and $\varepsilon=10$. As more centers are provided, it is harder to produce results to machine precision (see \ref{fig:linf-shape1-poly0-poly2} and \ref{fig:linf-shape10-poly0-poly2}): the ``search'' space is more dense and therefore harder to ``navigate''.
  \item For all these considerations, the better option seems to be $\varepsilon=1$. 
\end{itemize}



\begin{figure}[h]
  \includegraphics[width=\textwidth]{imagenes/experiments/1d/statistical_1d_full/u2/particular_cases/u2-TR7-C7-Kgaussian_kernel-Poly0-Equi-Sh1-E50000-4.csv-E50000-no-centers.pdf}
  \caption{Distribution of shape parameters at the end of the training process, for a Gaussian interpolator characterized by $C=7,k=1$ alongside a bias term, with starting shape parameter $\varepsilon=1$.}
  \label{fig:u2-TR7-C7-Kgaussian_kernel-Poly0-Equi-Sh1-E50000-4}
\end{figure}

\begin{figure}[h]
  \includegraphics[width=\textwidth]{imagenes/experiments/1d/statistical_1d_full/u2/particular_cases/u2-TR7-C7-Kgaussian_kernel-Poly0-Equi-Sh10-E50000-34.csv-E50000-no-centers.pdf}
  \caption{Distribution of shape parameters at the end of the training process, for a Gaussian interpolator characterized by $C=7,k=1$ alongside a bias term, with starting shape parameter $\varepsilon=10$.}
  \label{fig:u2-TR7-C7-Kgaussian_kernel-Poly0-Equi-Sh10-E50000-34}
\end{figure}

\clearpage

\textbf{Further refinements}

Despite achieving satisfactory results across our previous experiments, one can now reflect on what could be refined:
\begin{enumerate}
  \item During the preliminary experimentation, we worked with Chebyshev and equispaced nodes. In a ``realistic'' setting, where one would be provided experimental data points, one cannot expect the points to be sampled this way. \label{proposal:uniform-points}
  \begin{itemize}
    \item Would our method still work for a set of points that we do not anticipate? \item How would the statistics of our model look like?
  \end{itemize}
  \item What is the reason behind starting out with a certain value of $\varepsilon$?
  Figure \ref{fig:distribution-alphas-u2} shows that, regardless of the starting value, the shape parameters tend to be localized most notoriously close to the starting point. For instance, as $C$ and $k$ are increased, one can expect $\varepsilon=1$ to eventually produce ill-posed problems. \label{proposal:estimate-epsilon}
  \item Loss curves like those of Figures \ref{fig:u2-TR7-C7-Kgaussian_kernel-Poly0-Equi-Sh1-E50000-4} and \ref{fig:u2-TR7-C7-Kgaussian_kernel-Poly0-Equi-Sh10-E50000-34} feature high oscillations. This ultimately raises some questions on convergence, which we attempted to solve by looking at the best epochs' ratio, which is a non-standard metric. \label{proposal:scheduler} \begin{itemize}
    \item It is desirable to find a training regime whereby the ``speed'' of the method is preserved while oscillations are reduced.
  \end{itemize}
\end{enumerate}

To solve these problems, we introduce the following refinements to our methodology and repeat the exact same previous experiment:
\begin{enumerate}
  \item Training points should be sampled from the uniform distribution, whose upper and lower bounds will be those of the interval of the function we wish to approximate.
  \item Let $C$ be the number of equispaced centers. For each value of $C$, we consider the matrix-form interpolation problem where the interpolation points are also our centers. Using the bisection method, we look for $\varepsilon$ such that the condition number of the interpolation matrix is between $10^{-6}$ and $10^{-7}$. For each value of $C$, we will be using the previously-computed $\varepsilon$ as an estimation of a suitable starting shape value.
  \item A suitable candidate for preserving the ``speed'' of our method while reducing oscillations, especially towards the end of the training process, is the \href{https://pytorch.org/docs/stable/generated/torch.optim.lr_scheduler.ReduceLROnPlateau.html}{\texttt{ReduceLROnPlateau}} PyTorch scheduler, which verifies if the loss function took values below \texttt{dynamic\_threshold} in \texttt{patience} iterations:
  \begin{itemize}
    \item If that is the case, \texttt{dynamic\_threshold} is reduced and we resume operation.
    \item If that is not the case, the learning rate is reduced by \texttt{factor}.
  \end{itemize}
  Preliminary experimentation showed that, in the 1D case, \texttt{patience} could be increased to $100$, whereas the rest of the scheduler's parameters are set as default.
\end{enumerate}

\textbf{Conclusions}

Including a scheduler into our methodology allows us to reliably compare the last iteration of our iterative method with the best iterations in the $L^\infty-$norm, as we remove oscillations on those verification metrics. Furthermore, we now incorporate other indicators like the $L^2$ norm as used in Machine Learning (the root mean-squared error on the verification dataset): as the study progressed, it became evident that this new metric could provide more detail into the learning process, the $L^\infty$ norm being too stringent for our purposes. 

The incorporation of this new metric has been shown to be illustrative, as Figure \ref{fig:statistic-u2-scheduler-l2-linf} shows, on its right hand side, that the $L^2$ norm continues to decline past the best iteration in the $L^\infty-$norm. The incorporation of a scheduler also allows us to compare the last iteration of our process in the $L^\infty-$norm with its minimum, showing that they are not far away from each other.

\begin{figure}[h]
  % \frame
  \hspace*{-1cm}
  {\includegraphics[width=.42\textwidth,clip=true,trim={0 0 2cm 0}]{imagenes/experiments/1d/statistical_1d_full_scheduler_extrapolation/comparison-linf.pdf}}
  % \frame
  {\includegraphics[width=.42\textwidth,clip=true,trim={0 0 2cm 0}]{imagenes/experiments/1d/statistical_1d_full_scheduler_extrapolation/comparison-l2.pdf}}
  % \frame
  {\includegraphics[width=.08\textwidth,clip=true,trim={12.1cm 1cm 0 1cm}]{imagenes/experiments/1d/statistical_1d_full_scheduler_extrapolation/comparison-l2.pdf}}
  \caption{Left-hand side: distributions of the $L^\infty-$norm across all experiments, comparing for each experiment (represented by a data point) its minimum with the value achieved at the last iteration. Right-hand side: comparison in the $L^2-$norm involving the value obtained in best iteration in the $L^\infty-$norm with the end value.}
  \label{fig:statistic-u2-scheduler-l2-linf}
\end{figure}

For further reference, Figure \ref{fig:shape-parameters-end-training} shows the distribution of parameters at the end of the training process. The legend shows the suitable estimated starting values for interpolating on an equispaced lattice of $C$ centers yielded by the bijection method.


\begin{figure}[h]
  % \frame
  
  {\includegraphics[width=.6\textwidth,clip=true,trim={0 0 0 0}]{imagenes/experiments/1d/statistical_1d_full_scheduler_extrapolation/distribution_of_shape_parameters_at_end_of_training.pdf}}
  \caption{Distribution of shape parameters at the end of the training process.}
  \label{fig:shape-parameters-end-training}
\end{figure}

\begin{figure}[h]
  % \frame
  {\includegraphics[width=.4\textwidth,clip=true,trim={0 0 0 0}]{imagenes/experiments/1d/statistical_1d_full_scheduler_extrapolation/medians_linf_u2_gaussian_kernel_Poly-1_uniform.pdf}}
  {\includegraphics[width=.4\textwidth,clip=true,trim={0 0 0 0}]{imagenes/experiments/1d/statistical_1d_full_scheduler_extrapolation/medians_linf_u2_gaussian_kernel_Poly2_uniform.pdf}}
  \caption{Distribution of shape parameters at the end of the training process.}
  \label{fig:medians-linf-scheduler-u2}
\end{figure}

\begin{figure}[h]
  % \frame
  {\includegraphics[width=.4\textwidth,clip=true,trim={0 0 0 0}]{imagenes/experiments/1d/statistical_1d_full_scheduler_extrapolation/medians_l2_u2_gaussian_kernel_Poly-1_uniform.pdf}}
  {\includegraphics[width=.4\textwidth,clip=true,trim={0 0 0 0}]{imagenes/experiments/1d/statistical_1d_full_scheduler_extrapolation/medians_l2_u2_gaussian_kernel_Poly2_uniform.pdf}}
  \caption{Distribution of shape parameters at the end of the training process.}
  \label{fig:medians-l2-scheduler-u2}
\end{figure}
...
\subsubsection*{2D function approximation}

We now approximate 2D functions under the same considerations proposed in the \textit{Further refinements} section. The only difference between the 1D case is the estimated starting values of $\varepsilon$ as well as the \texttt{patience} parameter of the scheduler, which is set to $30$ instead. 

In the following, we consider $(x-1/2)^2+(y-1/2)^2$, which we approximate in the interval $[-1, 1]$. We may refer in the following to this function as \texttt{parabola}. Figure \ref{fig:medians-experiment-parabola} shows the medians of the experiment (elapsed times and $L-$norms). Figure \ref{fig:incidence-l-norms-parabola} shows the incidence of the $L^\infty$ and $L^2-$norms on the verification datasets, showing that the best iterations of both norms are close to the last iteration we do. 

Figure \ref{fig:incidence-largest-error} shows that we commit the largest error in the $L^\infty-$norm along the border of the domain (and most recurrent in the corners). One is evidently less likely to find a training point located the border as it is a zero-measure set in $[-1,1]\times[-1,1]$.

Figure \ref{fig:shape-parameters-end-training-parabola} shows the distribution of the shape parameters at the end of the training process. For the sake of inspection, Figure \ref{fig:parabola-training-TR30-C225} shows an example of the training experiment involving $225$ centers and a scaling factor $k=4$, which indicates that the right combination of $\varepsilon$ and scheduler is still able to produce smooth learning curves.

  \begin{figure}[h]
    \begin{tabular}{ccc}
      \hspace*{-2.5cm}
      {\includegraphics[width=.4\textwidth, clip=true, trim={0 0 0 0}]{imagenes/experiments/2d/statistical_2d_full_scheduler/parabola/medians_linf_parabola_gaussian_kernel_Poly-1.pdf}}       & 
      {\includegraphics[width=.4\textwidth, clip=true, trim={0 0 0 0}]{imagenes/experiments/2d/statistical_2d_full_scheduler/parabola/medians_l2_parabola_gaussian_kernel_Poly-1.pdf}} &
      % \frame
      \includegraphics[width=.4\textwidth, clip=true, trim={0 0 0 0}]{imagenes/experiments/2d/statistical_2d_full_scheduler/parabola/medians_times_parabola_gaussian_kernel_Poly-1.pdf} 
                                                                                 \\
      % \frame
  \multicolumn{3}{c}{{\includegraphics[width=.8\textwidth, clip=true, trim={65cm 9.1cm 0 .2cm}]{imagenes/experiments/2d/statistical_2d_full_scheduler/legend.pdf}}} \\ 
    \end{tabular}
    \caption{Medians of $L^\infty-$norm (left), $L^2-$norm (center) and elapsed times (right) at the end of the training process for \texttt{parabola}, for a scaling factor $k$ in $|\text{\texttt{training\_points}}| = \lceil \sqrt{|\text{\texttt{centers}}| \cdot k} \rceil^2$.}
    \label{fig:medians-experiment-parabola}
  \end{figure}

\begin{figure}[h]
  \hspace*{-1cm}
  % \frame
  {\includegraphics[width=.47\textwidth, clip=true, trim={0 0 0 0 }]{imagenes/experiments/2d/statistical_2d_full_scheduler/parabola/incidence_of_linf_poly-1.pdf}}
  % \frame
  {\includegraphics[width=.45\textwidth, clip=true, trim={0 0 0 0 }]{imagenes/experiments/2d/statistical_2d_full_scheduler/parabola/incidence_of_l2_poly-1.pdf}}
  \includegraphics[width=.08\textwidth, clip=true, trim={12cm 0 0 0}]{imagenes/experiments/2d/statistical_2d_full_scheduler/parabola/cbar_incidence.pdf}
  \caption{Distributions of the $L-$norm for the function \texttt{parabola}. Left-hand side: distributions of the $L^\infty-$norm across all experiments, comparing for each experiment (represented by a data point) its minimum with the value achieved at the last iteration. Right-hand side: comparison in the $L^2-$norm involving the value obtained in best iteration in the $L^\infty-$norm with the end value.}
  \label{fig:incidence-l-norms-parabola}
\end{figure}

\begin{figure}[h]
  \includegraphics[width=.45\textwidth]{imagenes/experiments/2d/statistical_2d_full_scheduler/parabola/distribution_of_errors_L_inf_end.pdf}
  \caption{Histogram of the largest errors in $L^\infty-$norm across all experiments.}
  \label{fig:incidence-largest-error}
\end{figure}

\begin{figure}[h]
  \includegraphics[width=.6\textwidth,clip=true,trim={0 0 0 0}]{imagenes/experiments/2d/statistical_2d_full_scheduler/parabola/distribution_of_shape_parameters_at_end_of_training.pdf_poly-1.pdf}
  \caption{Distribution of shape parameters at the end of the training process.}
  \label{fig:shape-parameters-end-training-parabola}
\end{figure}

\begin{figure}[h]
  \includegraphics[width=\textwidth,clip=true,trim={0 0 0 0}]{imagenes/experiments/2d/statistical_2d_full_scheduler/parabola/parabola-TR30-C225-Kgaussian_kernel-Sh3.6621-18-E44371.pdf}
  \caption{Left-hand side: absolute error of the approximation of \texttt{parabola} with $C=225,k=4$, during its last iteration. Right-hand side: the corresponding learning curves.}
  \label{fig:parabola-training-TR30-C225}
\end{figure}

\clearpage
\newpage
\subsubsection*{Interpolation in 2D - \texttt{OLD?}}
