...
For each value of $C$, we compute a starting shape parameter (initially) common to all RBFs in the following manner: we first distribute the $C$ centers across an equispaced $d-$dimensional lattice. After that is done, choose $\varepsilon$ such that the condition number of the interpolation problem with interpolation points equal to the centers is between $10^{-6}$ and $10^{-7}$. One can do so, for instance, using the bisection method. For such shapes, the loss function will be observed to steadily decrease while achieving satisfactory performance on the verification dataset. Preliminary experimentation ruled out larger or smaller shape parameters for the following reasons:
\begin{itemize}
...
\begin{figure}
  % \frame
  \begin{tabular}{ccc}
  {\includegraphics[height=.25\textwidth]{imagenes/experiments/2d/heat_1d/heat_1d-TR100-C49-usual-E99548_modified.pdf}}&
  {\includegraphics[height=.25\textwidth]{imagenes/experiments/2d/heat_1d/heat_1d-TR100-C49-prueba_shape_normal-E3106078_modified.pdf}} &
  \quad {\includegraphics[height=.15\textwidth]{imagenes/experiments/2d/heat_1d/model_curves_legend.pdf}}  
  \end{tabular}
  \caption{Model curves of two attempts to solve the one-dimensional heat equation with $C=49$ and $100$ points per dimension, the left-hand side being with the methodology as described up until now and the right-hand side with the proposed change in the scheduler.}
  \label{fig:heat-1d-experiment-model-curves}
\end{figure}

For this particular equation, the methodology we have been using up until now has failed to produce any tractable results. If we turn to Figure \ref{fig:representation-of-the-solution-to-the-heat-equation}, the function features a steep derivative along time for $x=1/2$. As described in \cite{abrahamsen2019explicit}, it is key to provide a large amount of training points in that area to correctly reconstruct the solution. Therefore, we extend the amount of training points we consider, sampling instead $100$ points per dimension and present in the left-hand side of Figure \ref{fig:heat-1d-experiment-model-curves} the learning curves of an attempt at solving the problem featuring a total of $C=49$ centers and the corresponding usual 2D starting shape parameter $\varepsilon=2.6855$. 
