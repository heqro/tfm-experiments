...
Figure \ref{fig:medians-epochs-statistic-u2-poly} shows the medians of the best epochs' ratios as further polynomial terms are considered, whereas Figure \ref{fig:medians-linf-statistic-u2-poly} shows the corresponding medians in the $L^\infty-$norm on the verification dataset. On the basis of the $L^\infty-$norm, some conclusions can already be drawn:
...
\begin{figure}[H]
  ...
  \label{fig:medians-epochs-statistic-u2-poly}
\end{figure}
...
\begin{figure}[H]
  ...
  \label{fig:medians-linf-statistic-u2-poly}
\end{figure}
...
\begin{figure}[h]
  ...
  \label{fig:linf-shape10-poly0-poly3}
\end{figure}


% To this end, we consider  and $x \mapsto u_3(x)=(x-1/2)^3$ which we interpolate in the interval $[-1,1]$ under the same configurations of centers and training points as in the last set of experiments. 

% Furthermore, we equip the RBF interpolators with polynomial terms of order $2$ and $3$, respectively. 
