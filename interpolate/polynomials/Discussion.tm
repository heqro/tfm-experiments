...
<\body>
  ...
  <\equation*>
    p<rsub|n><around*|(|x|)>=<big|sum><rsub|i=0><rsup|n-1>a<rsub|i>x<rsup|i>
  </equation*>
  ...
  For <math|n+1=N>, \ there is a (unique) polynomial of degree <math|N-1>
  that passes through each <math|<around*|(|x<rsub|i>,f<around*|(|x<rsub|i>|)>|)>>.

  If the system is over-determined, i.e. <math|N\<gtr\>n+1>, note that\ 
